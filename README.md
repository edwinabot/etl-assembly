...
```bash
etl-assembly$ sam build --use-container
```
...
```bash
etl-assembly$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
etl-assembly$ sam local start-api
etl-assembly$ curl http://localhost:3000/
```
...
```bash
etl-assembly$ sam logs -n HelloWorldFunction --stack-name etl-assembly --tail
```
...
```bash
etl-assembly$ pip install pytest pytest-mock --user
etl-assembly$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name etl-assembly
```
